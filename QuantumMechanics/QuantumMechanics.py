...
dictionary_single_qubit_gates = {'I': Id, 'X2p': Sx * np.pi / 2, 'Xp': Sx * np.pi, 'X2m': -Sx * np.pi / 2,
                                 'Xm': -Sx * np.pi,
...
def transmission_to_beta_coefficients(transmission_vector, peA=1, peB=1):
    zthA = 1 - 2 * peA
    zthB = 1 - 2 * peB
    matrix = np.array([
...
def calculate_chi_from_density_matrices(list_density_matrices, initial_gates=default_initial_gates, peA=1, peB=1):
    if initial_gates == default_initial_gates and peA == 1 and peB == 1:
        ...
        for gate in initial_gates:
            ...
            # state = qutip.ket2dm(qutip.basis(2, 0))
            # state = qutip.tensor(state, state)
            state = qutip.tensor((1 - peA) * qutip.ket2dm(qutip.basis(2, 1)) + peA * qutip.ket2dm(qutip.basis(2, 0)),
                                 (1 - peB) * qutip.ket2dm(qutip.basis(2, 1)) + peB * qutip.ket2dm(qutip.basis(2, 0)))
            state = apply_gate(state, gate)
            state_basis += [state]
        ...
        for j in range(len(state_basis)):
            for n in range(len(operator_basis_two_qubits)):
                for m in range(len(operator_basis_two_qubits)):
                    beta[m, n, j, :] = decompose_in_basis(operator_basis_two_qubits[m] * state_basis[j] \
...
def AllXYMetric(dataset):
    if not (len(dataset) == 21):
        ...
        return (np.linalg.norm(dataset[0:5] - m1) + np.linalg.norm(dataset[5:17] - m2) + np.linalg.norm(
            dataset[17:21] - m3)) / (maximum - minimum)
